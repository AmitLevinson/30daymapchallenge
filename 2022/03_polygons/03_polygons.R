...
# clean is to filter out any stops *after* the train
all_rides_clean <- all_rides %>% 
  group_by(gtfs_ride_id) %>% 
  arrange(gtfs_ride_id, stop_sequence) %>% 
  mutate(train_stop = ifelse(str_detect(name, 'רכבת רחובות'), stop_sequence, NA)) %>% 
  tidyr::fill(train_stop, .direction = 'up') %>% 
  ungroup() %>% 
  filter(!is.na(train_stop))
  


get_route_ids <- function (ride_id) {
...
# Filter this line ref maybe? 33730
route_ids <-  map_dfr(unique(all_rides$gtfs_ride_id), get_route_ids )
route_info <- map_dfr(route_ids$gtfs_route_id, get_route_info)
route_info %>% 
  left_join(route_ids, by = c('id' = 'gtfs_route_id') ) %>% View()
...
train_stop <- bus_stops_sf %>% 
  ...
  geom_sf(data = rehovot_voronoi_poloygon_km, aes(fill = area), color = 'gray55', lwd = 0.1)+
  geom_sf(data = bus_stops_sf, inherit.aes = FALSE, size = 0.25, color = 'gray85')+
  geom_sf_text(data = train_stop, aes(label = label),
                   inherit.aes = F, family = 'Font Awesome 6 Free Solid', size = 4, show.legend = FALSE, color = 'white')+
  scale_fill_gradientn("Area (km<sup>2</sup>)", 
                       colors=viridis::inferno(10), breaks = c(0.25, 0.5,.75), na.value = 'gray95')+
  labs(title = "Nearest Bus Stop with a Direct Route to the Train",
         subtitle = "Nearest bus stop with a direct route to Rehovot's train station. All locations in an area<br>are closer to that bus stop (dot) than to any other stop. Bus stops and routes that reach<br>Rehovot's train station were sampled from GTFS data in the past two weeks.<br><span style='color:gray35;'>**Gray**</span> = NA for having mostly open fields.",
  ...
  theme(
    ...
    plot.subtitle = element_markdown(size = 18, lineheight = 1.1),
...
ggsave('03_polygons/bus_stops_updated.png', width = 13, height = 9)

ggplot(rehovot_voronoi_poloygon_km) +
  geom_histogram(aes(x= area)) +
  labs(title = 'Distribution of areas\' size to the nearest bus stop with a direct route\nto train',
       y= 'Count',
       x = 'Area size (km^2)')+
  theme_minimal() +
  theme(
    plot.title = element_text(size = 16, "Lora", face = 'bold'),
...
ggsave('03_polygons/area_distribution_updated.png', width = 8, height = 5)
